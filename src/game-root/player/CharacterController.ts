...
export class CharacterController {
    private gameEventManager: GameWindow;
    ...
    private _addkeylistener() {
        ...
        this.gameEventManager.addEventListener("keydown", this._handleKeyDown);
    }

    private _removekeylistener() {
        window.removeEventListener("keyup", this._handleKeyUp);
        this.gameEventManager.removeEventListener("keydown", this._handleKeyDown);
    }
    ...
    /**
     * avatar/角色可以由多个 mesh（网格）组成，并以层级方式组织。
     * 因此，我们会选择这个层级结构的根节点 mesh 作为 avatar（主控制网格）。
     * 根节点必须是 Mesh 类型，否则无法使用 moveWithCollision() 方法进行移动。
     *
     * 层级中的多个 mesh 可能都有骨骼（Skeleton）——通常它们会共享同一个骨骼。
     * 所以我们会选取第一个拥有骨骼的 mesh 的骨骼作为 avatar 的 Skeleton。
     *
     * @param avatar
     *        角色的主网格（必须是 Mesh）
     * @param camera
     *        用于控制视角的 ArcRotateCamera（如果为 null，默认视为 NPC 控制）
     * @param scene
     *        Babylon.js 的场景对象
     * @param actionMap / animationGroupMap
     *        用于将动作（如行走、跳跃）映射到动画的字典结构
     *        - 可以是 ActionMap（包含速度、声音等扩展属性）
     *        - 也可以是旧格式的 AnimationGroup Map（向后兼容）
     * @param faceForward
     *        是否让角色默认朝向正前方（用于设定初始朝向）
     */
    constructor(avatar: Mesh, camera: ArcRotateCamera, scene: Scene, gameEventManager: GameWindow, faceForward = false) {
...
export class ActionMap {
    public walk = new ActionData("walk", 4, "w");              // 行走动画，速度 3，按 W 键触发
    public walkBack = new ActionData("walkBack", 4, "s");    // 后退动画，速度 3，按 S 触发
    public walkBackFast = new ActionData("walkBackFast", 3, "na"); // 后退加速（未绑定按键）
    public idle = new ActionData("idle", 0, "na");             // 待机动画，无速度，无按键

    public idleJump = new ActionData("idleJump", 6, " ");      // 原地跳跃，速度 6，按空格触发
    public run = new ActionData("run", 6, "na");               // 奔跑动画（未绑定按键）
    public runJump = new ActionData("runJump", 6, "na");       // 奔跑跳跃动画

    public fall = new ActionData("fall", 0, "na");             // 下落动画（不播放）
    public turnLeft = new ActionData("turnLeft", Math.PI / 8, "na");  // 左转，每帧转 PI/8，A 键
    public turnLeftFast = new ActionData("turnLeftFast", Math.PI / 4, "na"); // 快速左转
    public turnRight = new ActionData("turnRight", Math.PI / 8, "na"); // 右转，D 键
    public turnRightFast = new ActionData("turnRightFast", Math.PI / 4, "na"); // 快速右转

    public strafeLeft = new ActionData("strafeLeft", 3, "a");     // 左移
    public strafeLeftFast = new ActionData("strafeLeftFast", 3, "na");
    public strafeRight = new ActionData("strafeRight", 3, "d");   // 右移
    public strafeRightFast = new ActionData("strafeRightFast", 3, "na");

    public slideBack = new ActionData("slideBack", 0, "na"); // 滑行
