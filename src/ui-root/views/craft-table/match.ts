...
// 滑动匹配算法
export function matchesPattern(playerGrid: Blocks[][], recipe: BlockRecipe): boolean {
    const pattern = recipe.pattern.map(row =>
        row.map(block => block ?? Blocks.Air)
    );
    ...
    for (let offsetY = 0; offsetY <= maxYOffset; offsetY++) {
        for (let offsetX = 0; offsetX <= maxXOffset; offsetX++) {
            ...
            // 镜像匹配
            if (recipe.allowMirrored && matchAtOffset(playerGrid, mirrorPattern(pattern), offsetX, offsetY)) {
                return true;
            }
        }
    }
...
// 在某个偏移量检查是否匹配
function matchAtOffset(grid: Blocks[][], pattern: Blocks[][], offsetX: number, offsetY: number): boolean {
    for (let y = 0; y < grid.length; y++) {
        for (let x = 0; x < grid[0].length; x++) {
            const inPatternArea =
                y >= offsetY && y < offsetY + pattern.length &&
                x >= offsetX && x < offsetX + pattern[0].length;
...
// 生成左右镜像版本的 pattern
function mirrorPattern(pattern: Blocks[][]): Blocks[][] {
