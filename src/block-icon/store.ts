...
export class BlockIconStore {
	...
	/** 存储图像 Blob */
	static async set(key: Blocks, blob: Blob): Promise<void> {
		const db = await this.db;
		await db.put("blocks", blob, key);
	}

	/** 获取图像 Blob */
	static async get(key: Blocks): Promise<Blob | undefined> {
		const db = await this.db;
		return await db.get("blocks", key);
	}

	/** 删除图像 */
	static async delete(key: Blocks): Promise<void> {
		const db = await this.db;
	...
	/** 判断图像是否存在 */
	static async has(key: Blocks): Promise<boolean> {
	 ...
	 * @param key
	 */
	static async getIconUrl(key: Blocks): Promise<string> {
		const blob = await this.get(key);
		return blob ? URL.createObjectURL(blob) : "";
	}

	/** 根据传入的 keys 获取所有对应的 icon URL */
	static async getIconUrls(keys: Blocks[]) {
		const db = await this.db;
		const tx = db.transaction("blocks", "readonly");
		const store = tx.objectStore("blocks");

		const result: Partial<Record<Blocks, string>> = {};
