...
class BlockFactory {
	...
	private classMap: Record<Blocks, typeof Block> = {} as any;
	private recipeMap: Record<Blocks, BlockRecipe[]> = {} as any; // 存储配方
	...
	public getBlockClass(type: Blocks) {
		return this.classMap[type];
	}

	// 注册单个方块类
	public register(type: Blocks, blockClass: any) {
	...
	public createBlock(scene: Scene, type: Blocks, pos: Vector3): Block {
		...
		if (BlockClass) {
			return new (BlockClass as any)(scene, pos);
		}
	...
	// 可选：允许手动注册额外方块类
	public registerManually(type: Blocks, blockClass: any) {
		this.classMap[type] = blockClass;
	...
	private async registerAll() {
		...
		for (const [_, mod] of Object.entries(modules)) {
			...
			if (BlockClass && BlockClass.__isEntityBlock) {
				this.classMap[BlockClass.__blockType as Blocks] = BlockClass;
				// 自动绑定配方
				this.registerRecipes(BlockClass);
			}
		}
